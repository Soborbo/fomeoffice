...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="text-align: center; padding: 20px; color: #999; font-size: 12px;">
      <p style="margin: 0;">&copy; ${new Date().getFullYear()} Painless Van & Car Valeting Ltd</p>
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background-color: #ffffff; padding: 40px 30px; border-radius: 0 0 12px 12px;">
      ...
      <div style="margin-top: 30px; padding: 20px; background-color: #f8f9fa; border-radius: 8px; text-align: center;">
        <a href="mailto:${email}?subject=Re: Your message to Painless Car %26 Van Wash" style="display: inline-block; background-color: #19576d; color: #ffffff; padding: 12px 30px; border-radius: 25px; text-decoration: none; font-weight: 600;">
          Reply to Customer
        </a>
      </div>
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const officeEmail = await resend.emails.send({
      from: 'Website Contact <contact@bristolcarwash.co.uk>',
    ...
    const customerEmail = await resend.emails.send({
      from: 'Painless Car & Van Wash <contact@bristolcarwash.co.uk>',
      to: data.email,
      subject: 'We received your message - Painless Car & Van Wash',
