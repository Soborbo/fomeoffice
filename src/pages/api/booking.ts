...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="text-align: center; padding: 20px; color: #999; font-size: 12px;">
      <p style="margin: 0;">© ${new Date().getFullYear()} Painless Van & Car Valeting Ltd (trading as Foam Office)</p>
...
export const POST: APIRoute = async (context) => {
  ...
  const customerEmailPromise = resend
    ? resend.emails.send({
        from: 'Foam Office <bookings@foamoffice.co.uk>',
        to: data.email,
  ...
  const officeEmailPromise = resend
    ? resend.emails.send({
        from: 'Website Bookings <bookings@foamoffice.co.uk>',
